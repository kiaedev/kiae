...
template: {
	...
	output: {
		...
		kind:       "Deployment"
		spec: {
			selector: matchLabels: "app.yaml.oam.dev/component": context.name
				...
				metadata: {
					labels: {
						...
						if parameter.addRevisionLabel {
							"app.yaml.oam.dev/appRevision": context.appRevision
						}
						"app.yaml.oam.dev/component": context.name
						"app.yaml.oam.dev/revision":  context.revision
					}
	...
	outputs: {
		if len(exposePorts) != 0 {
			webserviceExpose: {
				...
				spec: {
					selector: "app.yaml.oam.dev/component": context.name
					ports: exposePorts
